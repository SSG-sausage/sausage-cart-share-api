...
public class CartShareController {
    ...
    @Operation(summary = "[external] 장바구니 신규 알림 개수 조회", responses = {
            @ApiResponse(responseCode = "200", description = "장바구니 신규 알림 개수 조회 성공입니다."),
            @ApiResponse(responseCode = "500", description = "예상치 못한 서버 에러가 발생하였습니다.", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    })
    @GetMapping("/cart-share/noti/cnt")
    public ResponseEntity<SuccessResponse<CartShareNotiCntResponse>> findCartShareNotiCnt(
            @Parameter(in = ParameterIn.HEADER) @MbrId Long mbrId) {
        return SuccessResponse.success(SuccessCode.FIND_CART_SHARE_NOTI_CNT_SUCCESS,
                cartShareService.findCartShareNotiCnt(mbrId));
    }
